...
local Window = Library:CreateWindow({ Title = '                     $ Sem.lol | Beta $                     ', AutoShow = true, TabPadding = 15, MenuFadeTime = 0.2 })
...
local killSayMessages = {
    "Sem.lol is a free script and u die to it..", 
    "Must be hard without Sem.lol 💔", 
    "Why aim when Sem.lol does it for you?",
    "Bros not on Sem.lol already 😂",
    "Cant be me icl",
...
function toggleDesync(state)
    ...
    if desync.enabled then
        ...
        Library:Notify("Desync Enabled '" .. desync.mode .. "' Sem.lol $", 2)
    else
        resetCamera()
        Library:Notify("Desync Disabled '" .. desync.mode .. "' Sem.lol  $", 2)
    end
end
...
local webhook = Modifications:AddButton('Redeem Codes', function()
   ...
   for _, code in pairs(codes) do
       ...
       Library:Notify("Trying code: " .. code .. " Sem.lol | Private", 5)
       task.wait(4.2)
   end
end)
...
getgenv().PlayerActions:AddButton('Execute Action', function()
    ...
    if targetPlayer and targetPlayer.Character then
        ...
        if tool and tool:FindFirstChild("Ammo") then
            ...
            Library:Notify("Equip a tool to use this function. | Sem.lol", 5)
        end
    end
end)
...
local WatermarkConnection = RunService.RenderStepped:Connect(function()
    ...
    Library:SetWatermark(("[ Sem.lol ] | $ Beta $ |  %s | %s (%d) | Uptime: %s | FPS %d | %d ms"):format(
